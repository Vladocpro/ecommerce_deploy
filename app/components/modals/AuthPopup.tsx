...
const AuthPopup = () => {
   ...
   return (
       <div className={`flex items-center justify-center fixed z-20 inset-0 transition-all duration-500 ${authPopup ? "visible" : "invisible opacity-0"}`}>
          ...
          <form
              className={`relative  w-80 py-5 flex flex-col bg-white rounded-lg transition-all duration-500 ${authPopup ? "translate-y-0" : "-translate-y-32 opacity-0"}`}
             ...
             <input placeholder="Email" type="text" id="email"
                    {...register('email', {
                       ...
                       minLength: {value: 11, message: "Specify valid Email"},
             ...
             <label htmlFor="password"
                    className={`text-red-500 ml-6 mr-5 h-4 block text-xs opacity-0 ${errors.password?.message !== undefined && "opacity-100"}`}>
             ...
             <button type="submit" className="py-[10px] mx-5 mt-2 font-medium rounded-lg bg-black  text-white ">
             ...
             <button className="relative py-[4px] mx-5 mt-3 px-4  border-2 border-black  rounded-lg hover:bg-black hover:text-white transition-all duration-300"
